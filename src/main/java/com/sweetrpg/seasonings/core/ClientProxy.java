...
public class ClientProxy extends CommonProxy
{
    ...
    public void registerItemVariantModel(Item item, String name, int metadata)
    {
        ...
        ModelLoader.registerItemVariants(item, new ResourceLocation("seasonings:" + name));
        ModelLoader.setCustomModelResourceLocation(item, metadata, new ModelResourceLocation(Seasonings.MOD_ID + ":" + name, "inventory"));
    }
    ...
    public void registerItemSided(Item item)
    {
        ...
        if (item.getHasSubtypes())
        {
            ...
            for (ItemStack subItem : subItems)
            {
                ...
                ModelLoader.registerItemVariants(item, new ResourceLocation(Seasonings.MOD_ID, subItemName));
                ModelLoader.setCustomModelResourceLocation(item, subItem.getMetadata(), new ModelResourceLocation(Seasonings.MOD_ID + ":" + subItemName, "inventory"));
            }
        }
        else
        {
            ModelLoader.setCustomModelResourceLocation(item, 0, new ModelResourceLocation(Seasonings.MOD_ID + ":" + item.delegate.name().getResourcePath(), "inventory"));
        }
    }
